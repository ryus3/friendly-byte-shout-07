...
export async function createModonOrder(orderData, token) {
  try {
    ...
    //   - رفض الرصيد للطلبات السالبة (الإرجاع/الاستبدال): رسالة واضحة للمستخدم.
    const msgStr = String(data.msg || '');
    const isBalanceError = /رصيد|balance|negative|سالب/i.test(msgStr);
    if (isBalanceError) {
      throw new Error(
        `مدن: ${msgStr || 'الرصيد غير كافٍ لإنشاء طلب سالب'}. ` +
        'هذا الطلب يحتوي على مبلغ سالب (إرجاع/استبدال) ويتطلب رصيداً متاحاً في حساب مدن. ' +
        'يرجى شحن الحساب أو تحويل الطلب إلى طلب موجب.'
      );
    }
    if (data.errNum === 21 || data.errNum === '21') {
      throw new Error(
        msgStr
          ? `مدن: ${msgStr}. تأكد من تسجيل الدخول إلى مدن من إدارة شركات التوصيل، ومن أن الحساب لديه صلاحية إنشاء الطلبات.`
