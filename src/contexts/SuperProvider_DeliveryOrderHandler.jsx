...
// ✅ دالة عادية لا تستخدم hooks - يتم تمرير createUnifiedOrder كمعامل
export const handleDeliveryPartnerOrder = async (
  aiOrder, 
  itemsInput, 
  destination, 
  selectedAccount, 
  accountData = null,
  createUnifiedOrder // ✅ استقبال الدالة كمعامل
) => {
  try {
    ...
    if (result.success) {
      ...
      const resolveCurrentUserUUID = () => {
        const userStr = localStorage.getItem('sb-tkheostkubborwkwzugl-auth-token');
        if (userStr) {
          try {
