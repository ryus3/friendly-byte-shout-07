...
export const ProfitsProvider = ({ children }) => {
  ...
  const fetchProfitsData = useCallback(async () => {
    ...
    try {
      ...
      const [profitsRes, expenseInvoicesRes] = await Promise.all([
        supabase.from('profits').select('*').order('created_at', { ascending: false }),
        // جلب فواتير التحاسب من جدول المصاريف (مستحقات الموظفين) فقط
        supabase
          .from('expenses')
          .select('*')
      ...
      // توحيد مصدر فواتير التحاسب إلى جدول المصاريف فقط (raw)
      setSettlementInvoices(expenseInvoicesRes.data || []);
    } catch (error) {
