...
export const SuperProvider = ({ children }) => {
  ...
  const getVariantDetails = useCallback((variantId) => {
    ...
    for (const product of allData.products) {
      // البحث في product_variants (الهيكل الصحيح)
      if (product.product_variants) {
        const variant = product.product_variants.find(v => v.id === variantId);
        if (variant) {
          // الحصول على اللون من البيانات المرتبطة مباشرة
          const color_name = variant.colors?.name;
          const color_hex = variant.colors?.hex_code;
          
          // الحصول على القياس من البيانات المرتبطة مباشرة
          const size_name = variant.sizes?.name;
