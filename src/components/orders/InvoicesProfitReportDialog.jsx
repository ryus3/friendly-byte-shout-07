      ...
      <DialogContent className="max-w-3xl max-h-[94vh] overflow-hidden flex flex-col p-0 gap-0" dir="rtl">
        ...
        <div className="flex-1 overflow-hidden relative" ref={tabsContainerRef}>
          <motion.div
            ...
            dragElastic={0.15}
            onDragEnd={(_, info) => {
              if (info.offset.x < -60) goTab(tabIndex + 1);
              else if (info.offset.x > 60) goTab(tabIndex - 1);
            }}
            style={{ width: `${TABS.length * 100}%` }}
          >
            {/* Summary */}
            <div className="shrink-0 w-full overflow-y-auto px-4 py-4 space-y-3" style={{ width: `${100 / TABS.length}%` }}>
              {computing ? (
            ...
            {/* Invoices */}
            <div className="shrink-0 w-full overflow-y-auto px-4 py-3" style={{ width: `${100 / TABS.length}%` }}>
            ...
            {/* Distribution */}
            <div className="shrink-0 w-full overflow-y-auto px-4 py-3 space-y-4" style={{ width: `${100 / TABS.length}%` }}>
              <div>
