...
const OrdersStats = ({ orders, aiOrders, onAiOrdersClick, onStatCardClick }) => {
  ...
  const statsData = useMemo(() => [
    ...
    { key: 'processing', title: 'يحتاج معالجة', icon: Package, colors: ['orange-500', 'amber-600'], value: getStats('processing'), onPeriodChange: (p) => handlePeriodChange('processing', p), onClick: createClickHandler('processing'), periods: { today: 'اليوم', week: 'آخر أسبوع', month: 'آخر شهر', all: 'كل الوقت'} },
    ...
    { key: 'returned', title: 'راجعة', icon: CornerDownLeft, colors: ['red-500', 'rose-600'], value: getStats('returned'), onPeriodChange: (p) => handlePeriodChange('returned', p), onClick: createClickHandler('returned'), periods: { today: 'اليوم', week: 'آخر أسبوع', month: 'آخر شهر', all: 'كل الوقت'} },
