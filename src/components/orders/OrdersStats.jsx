...
const OrdersStats = ({ orders, aiOrders, onAiOrdersClick, onStatCardClick }) => {
  ...
  const statsData = useMemo(() => [
    ...
    { key: 'pending', title: 'قيد الانتظار', icon: Clock, colors: ['yellow-500', 'orange-500'], value: getStats('pending'), onPeriodChange: (p) => handlePeriodChange('pending', p), onClick: createClickHandler('pending'), periods: { today: 'اليوم', week: 'آخر أسبوع', month: 'آخر شهر', all: 'كل الوقت'} },
    ...
    { key: 'processing', title: 'تحتاج معالجة', icon: AlertTriangle, colors: ['red-500', 'rose-500'], value: getStats('processing'), onPeriodChange: (p) => handlePeriodChange('processing', p), onClick: createClickHandler('processing'), periods: { today: 'اليوم', week: 'آخر أسبوع', month: 'آخر شهر', all: 'كل الوقت'} },
