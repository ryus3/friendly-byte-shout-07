               ...
               <div className="space-y-3">
                 {(order.order_items || order.items || []).map((item, index) => {
                  ...
                  <div className="mt-4 pt-4 border-t border-border space-y-2">
                    {(() => {
                      // السعر الأصلي الثابت = final_amount
                      const originalTotal = parseFloat(order.final_amount) || 0;
                      const deliveryFee = parseFloat(order.delivery_fee) || 0;
                      const originalProductsPrice = originalTotal - deliveryFee;
                      
                      // السعر الحالي للمنتجات
                      const currentProductsPrice = parseFloat(order.total_amount) || 0;
                      
                      const hasDiscount = (order.discount || 0) > 0;
                      const hasIncrease = (order.price_increase || 0) > 0;
                      const hasPriceChange = hasDiscount || hasIncrease;
                      
                      return (
                        <>
                          {/* السعر الأصلي الكامل - يظهر فقط إذا كان هناك تغيير */}
                          {hasPriceChange && originalTotal > 0 && (
                            <div className="flex justify-between items-center text-sm">
                              <span className="text-muted-foreground">السعر الأصلي الكامل</span>
                              <span className="text-foreground">{originalTotal.toLocaleString()} د.ع</span>
                            </div>
                          )}
                          ...
                          <div className="flex justify-between items-center text-sm">
                            ...
                            <span className="font-medium text-foreground">
                              {(hasPriceChange ? originalProductsPrice : currentProductsPrice).toLocaleString()} د.ع
                            </span>
                          </div>
                          ...
                          <div className="flex justify-between items-center pt-2 border-t">
                            ...
                            <span className="text-xl font-bold text-primary">
                              {(currentProductsPrice + deliveryFee).toLocaleString()} د.ع
                            </span>
                          </div>
                        </>
                      );
