    ...
    try {
      ...
      const { data, error } = await supabase
        ...
        .select(`
          ...
          created_at,
          updated_at,
          profiles!telegram_employee_codes_user_id_fkey(user_id, display_name, email, is_active)
        `)
        ...
        <div className="flex-1 overflow-y-auto space-y-4 sm:space-y-6 py-2 sm:py-4 px-1 sm:px-0">
          ...
          <div className="bg-card border rounded-lg p-3 sm:p-4">
            ...
            <div className="text-xs sm:text-sm text-muted-foreground space-y-2">
              ...
              >
                {showConnectionInfo ? 'إخفاء' : 'عرض'} كيفية الاتصال
              </Button>
              
              {showConnectionInfo && (
                <div className="mt-3 p-3 bg-muted rounded-lg text-xs space-y-2">
                  <p><strong>للاتصال بالبوت:</strong></p>
                  <p>1. ابحث عن البوت: <code>@{botStatus.username || 'Ryusiq_bot'}</code></p>
                  <p>2. ابدأ محادثة بإرسال <code>/start</code></p>
                  <p>3. استخدم رمزك الشخصي لإرسال الطلبات</p>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="bg-card border rounded-lg p-3 sm:p-4">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4">
              <h3 className="text-sm sm:text-lg font-medium flex items-center">
                ...
                {user?.role === 'super_admin' || user?.role === 'manager' ? 'رموز جميع الموظفين' : 'الرمز الشخصي'}
              </h3>
                    ...
                    <div className="flex items-center gap-3 flex-1 min-w-0">
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-xs sm:text-sm font-medium text-foreground truncate">
                          {code.profiles?.display_name || code.profiles?.email || 'موظف غير محدد'}
                        </p>
