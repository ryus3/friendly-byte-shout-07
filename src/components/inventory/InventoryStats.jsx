...
const InventoryStats = ({ inventoryItems, lowStockCount, reservedStockCount, onFilterChange, onViewArchive, onRestoreProduct }) => {
  ...
  const stats = [
    ...
    { title: 'مخزون منخفض', value: lowStockCount, icon: AlertTriangle, colorClass: 'bg-gradient-to-tr from-amber-600 to-yellow-500', delay: 0.4, onClick: () => onFilterChange('low') },
    { title: 'مخزون نافذ', value: outOfStockCount, icon: PackageX, colorClass: 'bg-gradient-to-tr from-red-600 to-red-900', delay: 0.5, onClick: () => onFilterChange('out-of-stock') },
  ];
