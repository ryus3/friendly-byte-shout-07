...
const StatCard = ({ title, value, icon: Icon, colors, format, onPeriodChange, currentPeriod, onClick, periods, onEdit, children }) => {
  ...
  let formattedValue;
  if (format === 'currency') {
      formattedValue = `${displayValue.toLocaleString()} د.ع`;
      ...
      <Card className={cn(
        ...
        <CardHeader className="flex flex-row items-start justify-between space-y-0 pb-2 relative z-10">
          ...
          <div className="absolute top-2 left-2 z-20 flex gap-1">
            {onEdit && (
              <Button variant="ghost" size="icon" className="w-7 h-7 text-muted-foreground hover:bg-muted/50" onClick={(e) => { e.stopPropagation(); onEdit(); }}>
            ...
            {onPeriodChange && (
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <Button variant="ghost" size="icon" className="w-7 h-7 text-muted-foreground hover:bg-muted/50" onClick={(e) => e.stopPropagation()}>
        ...
        <CardContent className="flex-grow flex flex-col justify-between relative z-10">
            {children ? (
              ...
              >
                ...
                <motion.div 
                  className={cn(
                      "w-12 h-12 shrink-0 rounded-lg flex items-center justify-center text-white transition-all duration-300",
                      "bg-gradient-to-br",
