-- تعديل دالة process_telegram_order لإصلاح التعرف على مرادفات المدن
-- النسخة ذات 7 معاملات: (p_employee_code, p_message_text, p_telegram_chat_id, p_city_id, p_region_id, p_city_name, p_region_name)
...
DECLARE
  ...
  v_first_line text;
  v_first_word text;  -- 🆕 متغير جديد للكلمة الأولى
...
BEGIN
  ...
  IF v_is_city THEN
    -- إذا كانت الكلمة الأولى مدينة أو مرادف مدينة، استخدم الاسم الافتراضي
    v_customer_name := v_default_customer_name;
  ELSIF v_first_line IS NOT NULL AND v_first_line != '' THEN
    -- إذا لم تكن مدينة وغير فارغ، استخدمه كاسم الزبون
