-- استرجاع النسخة الذكية الكاملة من extract_product_items_from_text
-- التي كانت تعمل بشكل صحيح في 8 أكتوبر 2025

-- حذف النسخة الحالية المبسطة
DROP FUNCTION IF EXISTS public.extract_product_items_from_text(text);

-- إنشاء النسخة الذكية الكاملة (439 سطر)
...
DECLARE
  ...
  v_line_has_product boolean := false;
  i integer;
BEGIN
  ...
  LOOP
    ...
    LOOP
      ...
      -- البحث عن القياس (مع التمييز الذكي)
      FOREACH v_word IN ARRAY v_words
      LOOP
        -- أولاً: هل الكلمة رقم؟
        IF v_word ~ '^[0-9]+$' THEN
          -- التحقق من جدول sizes
          ...
          END IF;
        ELSE
          -- البحث عن القياسات النصية (S, M, L, XL, إلخ)
          SELECT s.id, s.name INTO v_found_size
          FROM sizes s 
      ...
      IF (NOT v_color_requested OR v_found_color.id IS NOT NULL) 
        ...
        SELECT pv.id, pv.base_price as price, COALESCE(i.quantity - i.reserved_quantity, 0) as available_stock
        INTO v_variant
    ...
    IF v_size_count_in_line = 1 AND v_found_size_for_line.id IS NOT NULL AND array_length(v_temp_items, 1) > 1 THEN
      ...
      FOR i IN 1..array_length(v_temp_items, 1) LOOP
        IF (v_temp_items[i]->>'size') IN ('افتراضي', 'غير محدد') THEN
          -- إعادة التحقق من المخزون بالقياس الجديد
          SELECT pv.id, pv.base_price as price, COALESCE(inv.quantity - inv.reserved_quantity, 0) as available_stock
          INTO v_variant
          FROM product_variants pv
    ...
    -- إضافة المنتجات من السطر إلى النتيجة النهائية فقط إذا كان السطر يحتوي على منتج
    IF v_line_has_product AND array_length(v_temp_items, 1) > 0 THEN
      FOR i IN 1..array_length(v_temp_items, 1) LOOP
