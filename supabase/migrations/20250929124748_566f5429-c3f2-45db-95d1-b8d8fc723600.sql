-- تحسين رسائل الخطأ المحددة في دالة extract_product_items_from_text
...
BEGIN
  ...
  -- إنشاء رسائل الخطأ المحددة والواضحة
  ...
  RAISE NOTICE '📝 رسالة الخطأ المحددة: %', v_alternatives;
  
  -- إرجاع المنتج غير المتوفر مع رسالة الخطأ المحددة
  RETURN jsonb_build_array(
    jsonb_build_object(
      'product_name', v_found_product.name,
