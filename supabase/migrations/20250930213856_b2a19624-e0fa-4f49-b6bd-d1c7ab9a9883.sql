...
DROP FUNCTION IF EXISTS public.process_telegram_order(p_order_text text, p_employee_code text);
...
BEGIN
  ...
  IF p_employee_code IS NOT NULL THEN
    ...
    WHERE employee_code = p_employee_code  -- استخدام employee_code الصحيح
      AND is_active = true
    LIMIT 1;
