-- تحديث دالة استخراج المنتجات مع خاصية النظر للأمام لاستنتاج القياسات
...
BEGIN
  ...
  -- معالجة كل جزء على حدة باستخدام مؤشر للوصول للجزء التالي
  FOR v_part_index IN 1..array_length(v_parts, 1)
  LOOP
    v_part := v_parts[v_part_index];
    ...
    -- البحث عن الحجم في الجزء الحالي
    FOREACH v_word IN ARRAY v_words
    LOOP
    ...
    -- 🆕 خوارزمية النظر للأمام: البحث عن القياس في الجزء التالي
    IF NOT v_size_requested AND v_part_index < array_length(v_parts, 1) THEN
      v_next_part := v_parts[v_part_index + 1];
      v_next_words := string_to_array(lower(trim(regexp_replace(
        regexp_replace(v_next_part, E'[\r\n]+', ' ', 'g'),
        E'\\s+', ' ', 'g'
      ))), ' ');
      
      -- البحث عن حجم في الجزء التالي
      FOREACH v_next_word IN ARRAY v_next_words
      LOOP
        SELECT s.id, s.name INTO v_next_size
        FROM sizes s 
        WHERE lower(s.name) = v_next_word
           OR (v_next_word = 'ميديم' AND lower(s.name) = 'm')
           OR (v_next_word = 'لارج' AND lower(s.name) = 'l')
           OR (v_next_word = 'اكس' AND lower(s.name) = 'xl')
           OR (v_next_word = 'سمول' AND lower(s.name) = 's')
           OR lower(s.name) LIKE '%' || v_next_word || '%'
        LIMIT 1;
        
        IF v_next_size.id IS NOT NULL THEN
          v_found_size := v_next_size;
          v_size_requested := true;
          RAISE NOTICE '🔮 تم استنتاج الحجم من الجزء التالي: %', v_found_size.name;
          EXIT;
        END IF;
      END LOOP;
    END IF;
