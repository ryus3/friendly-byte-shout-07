...
DROP FUNCTION IF EXISTS public.process_telegram_order(uuid, jsonb, text, text, text, text, text, numeric, text);
DROP FUNCTION IF EXISTS public.process_telegram_order(text, jsonb, text, text, text, text, text, numeric);

-- إنشاء النسخة الصحيحة الوحيدة
...
DECLARE
  ...
  v_item jsonb;
BEGIN
  -- الحصول على معلومات الموظف من telegram_employee_codes
  SELECT 
    tec.user_id,
    COALESCE(p.full_name, tec.telegram_code, 'موظف تليغرام'),
    tec.default_customer_name
  INTO v_employee_id, v_employee_name, v_default_customer_name
  FROM public.telegram_employee_codes tec
  LEFT JOIN public.profiles p ON p.user_id = tec.user_id
  ...
  INSERT INTO public.ai_orders (
    ...
    'telegram',
    'pending',
    v_employee_id,  -- UUID بدلاً من النص
    p_message_text,
    jsonb_build_object(
