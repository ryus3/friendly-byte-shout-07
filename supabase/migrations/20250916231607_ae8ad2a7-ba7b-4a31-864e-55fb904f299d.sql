...
WHERE ao.created_at >= now() - interval '24 hours'
  AND n.id IS NULL; -- فقط الطلبات التي لا تحتوي على إشعارات
...
SELECT 
  ...
  NULL as user_id, -- إشعار عام للمديرين
  jsonb_build_object(
    'ai_order_id', ao.id,
    'customer_name', ao.customer_name,
    'total_amount', ao.total_amount,
...
WHERE ao.created_at >= now() - interval '24 hours'
  AND n.id IS NULL; -- فقط الطلبات التي لا تحتوي على إشعارات عامة

RAISE NOTICE 'تم إصلاح إعدادات Real-time وإنشاء الإشعارات المفقودة لطلبات الذكاء الاصطناعي';