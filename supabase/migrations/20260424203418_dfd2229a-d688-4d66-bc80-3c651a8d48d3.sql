...
BEGIN
  ...
  -- 2) إزالة كل الأحرف غير الرقمية تماماً (تشمل + و - و فراغات و أقواس)
  ...
  -- 3) المسح بنافذة منزلقة: نجرب نوافذ بأطوال 14, 13, 12, 11, 10
  i := 1;
  WHILE i <= stream_len - 9 LOOP
    normalized_phone := NULL;

    -- صيغة 14 رقم: 00964 + 7XXXXXXXXX
    IF i + 13 <= stream_len + 0 AND substring(digits_stream from i for 5) = '00964'
       AND substring(digits_stream from i + 5 for 1) = '7' THEN
      candidate := substring(digits_stream from i for 14);
      normalized_phone := '0' || substring(candidate from 6);
      i := i + 14;
    -- صيغة 13 رقم: 0964 + 7XXXXXXXXX (نادرة)
    ELSIF i + 12 <= stream_len AND substring(digits_stream from i for 4) = '0964'
      ...
      i := i + 13;
    -- صيغة 12 رقم: 964 + 7XXXXXXXXX
    ELSIF i + 11 <= stream_len AND substring(digits_stream from i for 3) = '964'
       AND substring(digits_stream from i + 3 for 1) = '7' THEN
      candidate := substring(digits_stream from i for 12);
    ...
    ELSIF i + 10 <= stream_len AND substring(digits_stream from i for 2) = '07' THEN
      ...
      i := i + 11;
    -- صيغة محلية بدون 0 (10 أرقام): 7XXXXXXXXX
    ...
    -- التحقق من صلاحية البادئة العراقية
