-- ✅ تحديث Trigger لإضافة المنتج للمخزون عند الحالة 17 للإرجاع
...
BEGIN
  ...
  -- ✅ معالجة خاصة للحالة 17 (رجع للتاجر) للطلبات الإرجاع
  IF status_code = '17' AND COALESCE(NEW.order_type, '') = 'return' THEN
    -- إرجاع المنتج للمخزون تلقائياً
    ...
    VALUES (
      'return_completed',
      'تم استلام إرجاع',
      'الطلب #' || COALESCE(NEW.order_number, NEW.id::text) || ' - تم إضافة المنتج للمخزون',
      NEW.created_by,
      jsonb_build_object(
        'order_id', NEW.id,
        'order_number', NEW.order_number,
