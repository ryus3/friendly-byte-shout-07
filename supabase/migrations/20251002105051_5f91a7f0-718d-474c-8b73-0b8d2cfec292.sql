-- تعديل دالة معالجة طلبات تليغرام - إزالة تقسيم المدينة/المنطقة
-- لإرسال العنوان الكامل للذكاء الاصطناعي
...
BEGIN
  ...
  -- 6. تحديد الاسم والعنوان (بدون تقسيم المدينة/المنطقة)
  IF v_has_city_in_line1 THEN
    v_customer_name := v_default_customer_name;
    v_city_raw := v_line1;  -- السطر الكامل كما هو: "بغداد دورة صحة"
  ELSIF v_line1 != '' THEN
  ...
  -- تم حذف كود التقسيم هنا ✅
  -- السطر الكامل سيُرسل للذكاء الاصطناعي كما هو

  -- 7. استخراج العنوان التفصيلي
  ...
  -- 9. فحص توفر المنتجات وحساب المبلغ الإجمالي
  ...
  INSERT INTO public.ai_orders (
    ...
    v_customer_name,
    v_phone,
    v_city_raw,  -- السطر الكامل بدون تقسيم ✅
    v_address,
    v_items,
    v_total_amount,
    v_delivery_fee,
  ...
  v_success_message := v_success_message || 
    '💵 المبلغ الإجمالي: ' || trim(to_char(v_total_amount, 'FM999,999')) || ' د.ع';
