-- إصلاح دالة معالجة طلبات التليغرام
DROP FUNCTION IF EXISTS process_telegram_order(text, bigint, jsonb);
...
BEGIN
  ...
  -- الحصول على معلومات الموظف باستخدام رمز التليغرام
  SELECT p.employee_code, p.user_id, p.full_name
  ...
  -- توليد رقم الطلب
  v_order_number := 'TG-' || TO_CHAR(now(), 'YYYYMMDD') || '-' || LPAD(nextval('order_sequence')::text, 4, '0');
  ...
  LOOP
    ...
    FROM products
    WHERE LOWER(name) LIKE '%' || LOWER(v_item->>'product_name') || '%'
      OR LOWER(sku) = LOWER(v_item->>'product_sku')
    ORDER BY 
      CASE WHEN LOWER(name) = LOWER(v_item->>'product_name') THEN 1 ELSE 2 END,
      similarity(LOWER(name), LOWER(v_item->>'product_name')) DESC
    LIMIT 1;
    ...
    IF v_product_record.id IS NULL THEN
      ...
      INSERT INTO products (
        ...
        COALESCE(v_item->>'product_sku', 'TG-' || gen_random_uuid()::text),
        v_employee_record.user_id,
        'telegram'
      ) RETURNING id, name, price, cost_price INTO v_product_record;
    END IF;

    -- البحث عن المتغير إذا كان محدد
    v_variant_record := NULL;
    IF v_item->>'color' IS NOT NULL OR v_item->>'size' IS NOT NULL THEN
      ...
      WHERE pv.product_id = v_product_record.id
        AND (v_item->>'color' IS NULL OR LOWER(c.name) = LOWER(v_item->>'color'))
        AND (v_item->>'size' IS NULL OR LOWER(s.name) = LOWER(v_item->>'size'))
      LIMIT 1;
    END IF;
...
EXCEPTION
  WHEN OTHERS THEN
    RETURN jsonb_build_object(
      ...
      'message', 'خطأ في معالجة الطلب: ' || SQLERRM
    );
END;
$$;