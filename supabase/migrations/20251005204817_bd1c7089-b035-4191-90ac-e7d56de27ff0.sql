-- إصلاح استعلام default_customer_name في دالة process_telegram_order
-- تغيير WHERE id = v_employee_id إلى WHERE user_id = v_employee_id

-- تحديث النسخة الأولى من الدالة (بدون معاملات المدينة)
...
BEGIN
  ...
  -- الحصول على اسم الزبون الافتراضي من ملف المستخدم (إصلاح الاستعلام)
  ...
  IF v_is_city THEN
    -- إذا كان السطر الأول مدينة، استخدم الاسم الافتراضي
    v_customer_name := v_default_customer_name;
  ELSIF v_first_line IS NOT NULL AND v_first_line != '' THEN
    -- إذا لم يكن مدينة وغير فارغ، استخدمه كاسم الزبون
    v_customer_name := v_first_line;
  ELSE
    -- في حالة أخرى، استخدم الاسم الافتراضي
    v_customer_name := v_default_customer_name;
  END IF;
  ...
  -- استخراج العنوان (بدون معالجة المدينة هنا)
  ...
  IF EXISTS (
    ...
    -- إرجاع رسالة البدائل من أول منتج غير متوفر
    DECLARE
...
-- تحديث النسخة الثانية من الدالة (مع معاملات المدينة والمنطقة)
...
BEGIN
  ...
  -- فحص إذا كان السطر الأول يبدأ بمدينة (وليس exact match)
  SELECT EXISTS(
    SELECT 1 FROM public.cities_cache 
