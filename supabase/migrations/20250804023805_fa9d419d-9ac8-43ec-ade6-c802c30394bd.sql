-- حذف الدالة الموجودة وإعادة إنشاؤها مع إصلاح مشكلة UUID
DROP FUNCTION IF EXISTS migrate_existing_customers_to_phone_loyalty();

-- إنشاء دالة دمج العملاء لنظام النقاط الصحيح
...
BEGIN
  ...
  FOR phone_group IN 
    SELECT 
      ...
      (ARRAY_AGG(c.id ORDER BY c.created_at))[1] as representative_customer_id,
