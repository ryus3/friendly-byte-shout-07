...
-- إصلاح دالة الفحص - استبعاد الطلبات المؤرشفة بشكل صحيح
...
BEGIN
  ...
  WITH calculated_values AS (
    SELECT 
      ...
      COALESCE(SUM(
        CASE 
          WHEN o.id IS NULL THEN 0  -- طلب مؤرشف/محذوف
          WHEN o.order_type = 'return' AND oi.item_direction = 'incoming' THEN 0
...
-- إصلاح دالة الإصلاح التلقائي بنفس المنطق
...
BEGIN
  ...
  updates AS (
    ...
    SET 
      reserved_quantity = COALESCE(cv.calc_reserved, 0),
      sold_quantity = COALESCE(cv.calc_sold, 0)
    FROM calculated_values cv
    JOIN product_variants pv ON pv.id = cv.variant_id
    JOIN products p ON p.id = pv.product_id
    WHERE inv.variant_id = cv.variant_id
      AND (inv.reserved_quantity != COALESCE(cv.calc_reserved, 0) 
           OR inv.sold_quantity != COALESCE(cv.calc_sold, 0))
    RETURNING 
      inv.variant_id,
      p.name AS product_name,
      inv.reserved_quantity AS old_reserved,
      cv.calc_reserved AS new_reserved,
      inv.sold_quantity AS old_sold,
