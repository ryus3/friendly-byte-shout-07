-- البحث عن جميع دوال process_telegram_order وحذفها
DO $$
DECLARE
    func_record RECORD;
BEGIN
    -- حذف جميع دوال process_telegram_order
    FOR func_record IN 
        SELECT p.oid, p.proname, pg_get_function_identity_arguments(p.oid) as args
        FROM pg_proc p 
        JOIN pg_namespace n ON p.pronamespace = n.oid 
        WHERE n.nspname = 'public' AND p.proname = 'process_telegram_order'
    LOOP
        EXECUTE 'DROP FUNCTION IF EXISTS public.process_telegram_order(' || func_record.args || ') CASCADE';
        RAISE NOTICE 'تم حذف الدالة: process_telegram_order(%)', func_record.args;
    END LOOP;
END $$;

-- إنشاء دالة إنشاء الطلبات الجديدة والصحيحة
