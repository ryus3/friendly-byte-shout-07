-- تعديل دالة process_telegram_order لتتحقق من مرادفات المدن عند استخراج اسم الزبون
...
BEGIN
  ...
  -- إذا السطر الأول لا يبدأ بمدينة (أو مرادف مدينة) ولا يحتوي أرقام، يعتبر اسم
  IF v_first_line != '' THEN
    -- التحقق من أن السطر الأول ليس مدينة أو مرادف مدينة
    SELECT (
      EXISTS (
        SELECT 1
        ...
        JOIN cities_cache cc ON cc.id = ca.city_id
        WHERE cc.is_active = true
          AND (
            lower(v_first_line) LIKE lower(ca.alias_name) || '%'
            OR lower(v_first_line) LIKE '%' || lower(ca.alias_name) || '%'
            OR lower(v_first_line) LIKE lower(ca.normalized_name) || '%'
            OR lower(v_first_line) LIKE '%' || lower(ca.normalized_name) || '%'
          )
      )
    ) INTO v_city_found;
