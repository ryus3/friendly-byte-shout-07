-- تحديث دالة استخراج المنتجات لاستخدام المتاح (quantity - reserved_quantity) بدلاً من المخزون الكلي
...
BEGIN
  ...
  LOOP
    -- التحقق من أن الكلمة رقم قصير (1-3 خانات فقط) وليس رقم هاتف طويل
  ...
  LOOP
    ...
    IF v_found_color.id IS NOT NULL THEN
      ...
      EXIT;
    ELSE
      -- التحقق من وجود كلمات ألوان غير معروفة
  ...
  LOOP
    ...
    IF v_found_size.id IS NOT NULL THEN
      ...
      EXIT;
    ELSE
      -- التحقق من وجود كلمات أحجام غير معروفة
  ...
  -- البحث عن المتغير المحدد - فقط إذا كان اللون والحجم متوفرين
  -- ✅ التعديل الأساسي: استخدام المتاح (quantity - reserved_quantity) بدلاً من المخزون الكلي
  ...
  -- إنشاء رسالة البدائل الذكية مع تفاصيل الألوان والأحجام
  -- ✅ التعديل: استخدام المتاح في البدائل
  ...
  ELSIF v_variant.id IS NOT NULL AND v_variant.available_stock < v_quantity THEN
    -- الكمية المطلوبة غير متوفرة - ✅ استخدام available_stock
    v_alternatives := format('❌ لم يتم إنشاء طلب!' || E'\n' ||
      'المنتج "%s" متوفر بكمية %s فقط (مطلوب %s)' || E'\n\n' ||
      '✅ الألوان والأحجام المتوفرة:' || E'\n%s', 
      v_found_product.name, v_variant.available_stock, v_quantity, COALESCE(v_smart_alternatives, 'لا توجد بدائل متوفرة'));
  ELSIF v_variant.id IS NULL THEN
    -- التركيبة المحددة (لون + حجم) غير متوفرة
  ...
  RAISE NOTICE '📝 رسالة الخطأ المبسطة: %', v_alternatives;
  
  -- إرجاع المنتج غير المتوفر مع رسالة الخطأ المبسطة
  RETURN jsonb_build_array(
    jsonb_build_object(
      'product_name', v_found_product.name,
