-- ==========================================
-- استرجاع النسخة الأصلية من extract_product_items_from_text
-- التي كانت تعمل بشكل ممتاز مع process_telegram_order
-- ==========================================
...
-- إعادة إنشاء الدالة الصحيحة (input_text parameter)
...
DECLARE
  ...
  v_size_mapping jsonb := '{
    "s": ["s", "سمول", "صغير", "small"],
    "m": ["m", "ميديم", "وسط", "متوسط", "medium"],
    "l": ["l", "لارج", "كبير", "large"],
    "xl": ["xl", "اكس لارج", "اكس", "xlarge", "x-large"],
    "xxl": ["xxl", "دبل اكس", "اكسين", "2xl", "2x", "xxlarge", "xx-large"],
    "xxxl": ["xxxl", "ثلاث اكسات", "3xl", "3x", "xxxlarge", "xxx-large"],
    "xs": ["xs", "اكس سمول", "xsmall", "x-small"]
  }'::jsonb;
  v_size_key text;
  v_size_synonyms jsonb;
  v_size_synonym text;
BEGIN
  ...
  LOOP
    ...
    -- البحث عن الحجم مع v_size_mapping الموسع
    FOREACH v_word IN ARRAY v_words
    LOOP
      -- البحث المباشر في جدول sizes
      SELECT s.id, s.name INTO v_found_size
      FROM sizes s 
      WHERE lower(s.name) = v_word
      LIMIT 1;
      
      -- إذا لم نجد، نبحث في v_size_mapping
      IF v_found_size.id IS NULL THEN
        FOR v_size_key, v_size_synonyms IN SELECT * FROM jsonb_each(v_size_mapping)
        LOOP
          FOR v_size_synonym IN SELECT jsonb_array_elements_text(v_size_synonyms)
          LOOP
            IF v_word = v_size_synonym THEN
              SELECT s.id, s.name INTO v_found_size
              FROM sizes s 
              WHERE lower(s.name) = v_size_key
              LIMIT 1;
      ...
      IF v_found_size.id IS NOT NULL THEN
        ...
        EXIT;
      ELSE
        IF v_word IN ('ميديم', 'لارج', 'سمول', 'اكس', 'دبل', 'كبير', 'صغير', 'وسط', 'xxxl', 'xxl', 'اكسين', 'ثلاث', 'اكسات') THEN
          v_size_requested := true;
