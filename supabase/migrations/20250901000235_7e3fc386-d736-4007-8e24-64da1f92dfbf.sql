-- Update Alwaseet status notifications: only notify for important statuses, normalize text statuses, and ensure dedup by code
...
BEGIN
  ...
  -- تطبيع الحالة إلى كود رقمي (مثلاً "فعال" => 1)
  ...
  -- الحصول على النص الحرفي للحالة من ALWASEET_STATUS_DEFINITIONS بناءً على الكود
  status_text := CASE status_code
    WHEN '0'  THEN 'معطل او غير فعال'
    WHEN '1'  THEN 'قيد التجهيز'
  ...
  -- تطبيق الصيغة: النص الحرفي + رقم التتبع
