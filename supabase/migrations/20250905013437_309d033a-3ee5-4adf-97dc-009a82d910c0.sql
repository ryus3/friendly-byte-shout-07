-- التحقق أولاً من وجود الطلبين
DO $$
DECLARE
    next_order_num integer;
BEGIN
    -- الحصول على أعلى رقم طلب
    SELECT COALESCE(MAX(CAST(SUBSTRING(order_number FROM 4) AS integer)), 0) + 1 
    INTO next_order_num 
    FROM orders 
    WHERE order_number ~ '^ORD[0-9]+$';

    -- استعادة الطلبين إذا لم يكونا موجودين
    ...
    SELECT * FROM (VALUES 
      ('100503893', '100503893', 'alwaseet', 'Ryusiq', 'delivered', 'تم التسليم للزبون', '91484496-b887-44f7-9e5d-be9db5567604'::uuid, 'عميل مسترد - استعادة', '07xxxxxxxxx', 'عنوان مؤقت', 'بغداد', 50000, '2025-08-01 10:00:00+00'::timestamptz, now(), 'ORD' || LPAD(next_order_num::text, 6, '0')),
      ('100579474', '100579474', 'alwaseet', 'Ryusiq', 'delivered', 'تم التسليم للزبون', '91484496-b887-44f7-9e5d-be9db5567604'::uuid, 'عميل مسترد - استعادة', '07xxxxxxxxx', 'عنوان مؤقت', 'بغداد', 50000, '2025-08-01 10:00:00+00'::timestamptz, now(), 'ORD' || LPAD((next_order_num + 1)::text, 6, '0'))
    ) AS new_orders
    WHERE NOT EXISTS (
      SELECT 1 FROM orders WHERE tracking_number IN ('100503893', '100579474')
    );
END $$;
