-- إصلاح دالة استخراج المنتجات - إزالة استعلام العمود غير الموجود
...
BEGIN
  ...
  -- البحث عن المنتجات في النص (بدون العمود price غير الموجود)
  FOR v_product IN 
    SELECT p.id, p.name
    FROM products p 
  ...
  LOOP
    ...
    IF lower(input_text) ILIKE '%' || lower(v_product.name) || '%' THEN
      ...
      SELECT pv.id, COALESCE(i.quantity, 0), COALESCE(pv.price, v_variant_price)
