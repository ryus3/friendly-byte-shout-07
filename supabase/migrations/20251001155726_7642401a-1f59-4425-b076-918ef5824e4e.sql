-- تحديث دالة معالجة طلبات تليغرام لتمييز بين الاسم والعنوان
...
BEGIN
  ...
  -- 2. قراءة الاسم الافتراضي من auth.users
  ...
  IF v_has_city_in_line1 THEN
    -- السطر الأول يحتوي على مدينة = عنوان
    v_customer_name := v_default_customer_name;
    v_city_raw := v_line1;
  ELSE
    -- السطر الأول = اسم، السطر الثاني = عنوان
    v_customer_name := COALESCE(NULLIF(v_line1, ''), v_default_customer_name);
