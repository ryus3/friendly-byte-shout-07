-- إصلاح دالة استخراج المنتجات من النص لتعمل مع base_price بدلاً من price
...
BEGIN
  ...
  LOOP
    ...
    FOR v_size IN 
      ...
      WHERE (
        lower(name) ILIKE '%' || v_normalized_word || '%'
        OR (lower(name) = 'medium' AND v_normalized_word IN ('ميديم', 'وسط', 'm'))
        OR (lower(name) = 'large' AND v_normalized_word IN ('لارج', 'كبير', 'l'))
        OR (lower(name) = 'small' AND v_normalized_word IN ('سمول', 'صغير', 's'))
        OR (lower(name) = 'xl' AND v_normalized_word IN ('اكس لارج', 'xl'))
        OR (lower(name) = 'xxl' AND v_normalized_word IN ('اكس اكس لارج', 'xxl'))
      )
      ORDER BY 
        CASE 
          WHEN lower(name) = v_normalized_word THEN 1
