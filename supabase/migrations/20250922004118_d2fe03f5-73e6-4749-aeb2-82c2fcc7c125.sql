-- إصلاح دالة process_telegram_order للعمل بالطريقة الصحيحة
DROP FUNCTION IF EXISTS process_telegram_order(jsonb, text, bigint);

-- إنشاء الدالة المحدثة والمصححة
CREATE OR REPLACE FUNCTION process_telegram_order(
  p_order_data jsonb,
  p_telegram_employee_code text, -- هذا هو الكود الذي يرسله البوت مثل RYU559
  p_chat_id bigint
)
RETURNS jsonb
LANGUAGE plpgsql
...
BEGIN
  ...
  -- الحصول على معلومات الموظف الصحيحة
  -- البحث عن الكود في telegram_employee_codes ثم الحصول على employee_code من profiles
  SELECT 
  ...
  WHERE tec.employee_code = p_telegram_employee_code  -- البحث بالكود الذي يرسله البوت
  ...
  IF v_employee_record.user_id IS NULL THEN
    RETURN jsonb_build_object(
      ...
      'message', 'الموظف غير موجود أو غير مصرح له - كود: ' || p_telegram_employee_code
    );
  END IF;
  ...
  -- توليد رقم الطلب باستخدام الدالة الصحيحة
  ...
  -- إنشاء سجل في ai_orders للربط مع نافذة إدارة الطلبات الذكية
  INSERT INTO ai_orders (
    ...
    p_order_data->>'original_text', v_employee_record.internal_employee_code
  ...
  RETURN jsonb_build_object(
    ...
    'final_amount', v_order_total + v_delivery_fee,
    'employee_info', jsonb_build_object(
      'telegram_code', v_employee_record.telegram_code,
      'internal_code', v_employee_record.internal_employee_code,
...
EXCEPTION
  WHEN OTHERS THEN
    RETURN jsonb_build_object(
      ...
      'employee_code_used', p_telegram_employee_code,
      'chat_id', p_chat_id
    );
END;
$$;