...
BEGIN
  ...
  IF v_found_product_id IS NULL THEN
    ...
    RETURN jsonb_build_array(
      jsonb_build_object(
        ...
        'total_price', 0,
        'is_available', false,
        'alternatives_message', 'لم يتم التعرف على أي منتج في النص المرسل. يرجى التأكد من اسم المنتج وإعادة المحاولة.'
      )
    );
  END IF;
  
  -- البحث عن معرفات اللون والحجم
  ...
  -- جمع الألوان مع أحجامها المتوفرة فعلياً (الخطة الجديدة)
  SELECT jsonb_object_agg(
    color_name, 
    json_agg(size_name ORDER BY 
      CASE size_name 
        WHEN 'S' THEN 1 
        WHEN 'M' THEN 2 
        WHEN 'L' THEN 3 
  ...
  FROM (
    SELECT DISTINCT c.name as color_name, s.name as size_name
    FROM product_variants pv
    INNER JOIN colors c ON pv.color_id = c.id
    INNER JOIN sizes s ON pv.size_id = s.id
    INNER JOIN inventory i ON pv.id = i.variant_id
  ...
  -- بناء رسالة البدائل الذكية (الخطة الجديدة)
  IF NOT v_exact_variant_available THEN
    ...
    -- عرض البدائل الذكية (كل لون مع أحجامه المتوفرة فعلياً)
    IF v_available_colors_sizes IS NOT NULL AND jsonb_typeof(v_available_colors_sizes) = 'object' THEN
      ...
      LOOP
        SELECT array_agg(value::text) INTO v_sizes_for_color
        FROM jsonb_array_elements_text(v_available_colors_sizes->v_color_name);
