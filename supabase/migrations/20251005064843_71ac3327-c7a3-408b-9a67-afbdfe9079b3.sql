-- تبسيط منطق استخراج اسم الزبون في process_telegram_order
...
BEGIN
  ...
  -- ✅ استخراج اسم الزبون بمنطق بسيط
  v_first_line := TRIM(SPLIT_PART(p_message_text, E'\n', 1));
  v_normalized_first_line := LOWER(TRIM(v_first_line));
  
  -- فحص إذا السطر الأول يحتوي على اسم مدينة
  IF v_first_line IS NOT NULL AND LENGTH(v_first_line) BETWEEN 2 AND 50 THEN
    SELECT EXISTS(
      SELECT 1 FROM cities_cache 
      WHERE is_active = true 
      AND (
        LOWER(name) = v_normalized_first_line 
        OR LOWER(name_ar) = v_normalized_first_line
        OR LOWER(v_normalized_first_line) LIKE '%' || LOWER(name) || '%'
        OR LOWER(name) LIKE '%' || v_normalized_first_line || '%'
      )
    ) INTO v_has_city;
  END IF;
  ...
  -- تحديد اسم الزبون
  IF v_has_city THEN
    -- السطر الأول فيه مدينة → استخدم الاسم الافتراضي
    v_customer_name := NULL;
  ELSIF v_first_line IS NOT NULL AND v_first_line != '' AND v_normalized_first_line !~ '^07[0-9]{9}$' THEN
    -- السطر الأول ليس مدينة وليس رقم هاتف → اعتبره اسم الزبون
  ...
  VALUES (
    ...
    jsonb_build_object(
      ...
      'customer_province', v_customer_city,
