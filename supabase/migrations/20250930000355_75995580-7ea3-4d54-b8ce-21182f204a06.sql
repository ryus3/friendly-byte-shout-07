-- Delete the current failing function version
DROP FUNCTION IF EXISTS public.process_telegram_order(input_text text, chat_id bigint);

-- Recreate the working version that expects structured jsonb data
...
-- Ensure extract_actual_address function exists (helper function)
...
AS $function$
BEGIN
  -- Simple address extraction - just clean the text
