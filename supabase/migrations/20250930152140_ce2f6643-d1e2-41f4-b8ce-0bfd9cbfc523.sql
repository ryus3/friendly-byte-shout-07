-- إصلاح استخراج المنطقة وأقرب نقطة دالة في process_telegram_order

CREATE OR REPLACE FUNCTION public.process_telegram_order(p_chat_id bigint, p_message_text text, p_employee_id uuid DEFAULT NULL::uuid)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  IF array_length(v_address_lines, 1) > 0 THEN
    ...
    -- فحص إذا كان يحتوي على مدينة (عتبة ثقة أقل = 0.6)
    ...
    IF NOT v_has_city AND NOT v_has_phone AND NOT v_has_product 
       AND length(v_first_line) > 0 AND length(v_first_line) < 50 THEN
      v_customer_name := v_first_line;
      RAISE NOTICE '✅ تم استخراج اسم الزبون من أول سطر: %', v_customer_name;
    END IF;
  END IF;

  -- إذا لم يتم استخراج اسم، استخدم الاسم الافتراضي
  ...
  -- تخفيض عتبة الثقة من 0.8 إلى 0.6
  ...
  -- البحث النهائي عن المدينة من السطر المحدد
  SELECT city_id, city_name INTO v_city_result
  FROM smart_search_city(v_address_line)
  ORDER BY confidence DESC
  LIMIT 1;
  ...
  IF v_city_result.city_id IS NOT NULL THEN
    ...
    -- البحث عن المنطقة في نفس السطر (تخفيض عتبة الثقة من 0.7 إلى 0.5)
    SELECT region_id, region_name, confidence INTO v_region_result
    ...
    IF v_region_result.region_name IS NOT NULL THEN
      ...
      RAISE NOTICE '🗺️ المنطقة المحددة: % (ثقة: %)', v_region_name, v_region_result.confidence;
    ELSE
      RAISE NOTICE '❌ لم يتم العثور على منطقة بثقة >= 0.5';
    END IF;
  ELSE
    RAISE NOTICE '❌ لم يتم تحديد المدينة';
  END IF;

  -- **معالجة ذكية محسّنة لاستخراج أقرب نقطة دالة**
  IF v_found_address_line AND v_address_line IS NOT NULL THEN
    RAISE NOTICE '🔍 بدء استخراج أقرب نقطة دالة من: %', v_address_line;
    
    -- الحصول على أسماء المنتجات للفلترة
    SELECT array_agg(lower(trim(name))) INTO v_product_names
    FROM products WHERE is_active = true;
    
    -- تقسيم السطر إلى كلمات مع الحفاظ على الترتيب
    ...
    LOOP
      ...
      IF length(v_word) < 2 THEN 
        RAISE NOTICE '⏭️ تجاهل كلمة قصيرة: %', v_word;
        CONTINUE; 
      END IF;
      
      -- تجاهل أرقام الهاتف (7 أرقام أو أكثر متتالية)
      IF v_word ~ '^[0-9]{7,}$' THEN 
        RAISE NOTICE '⏭️ تجاهل رقم هاتف: %', v_word;
        CONTINUE; 
      END IF;
      
      -- تجاهل اسم المدينة (مع مقارنة غير حساسة)
      IF v_city_name IS NOT NULL AND lower(v_word) = lower(v_city_name) THEN 
        RAISE NOTICE '⏭️ تجاهل اسم المدينة: %', v_word;
        CONTINUE; 
      END IF;
      
      -- تجاهل اسم المنطقة (مع مقارنة غير حساسة)
      IF v_region_name IS NOT NULL AND lower(v_word) = lower(v_region_name) THEN 
        RAISE NOTICE '⏭️ تجاهل اسم المنطقة: %', v_word;
        CONTINUE; 
      END IF;
      
      -- تجاهل أسماء المنتجات المعروفة
      IF v_product_names IS NOT NULL AND lower(v_word) = ANY(v_product_names) THEN
        RAISE NOTICE '⏭️ تجاهل اسم منتج: %', v_word;
        CONTINUE;
      END IF;
      
      -- تجاهل أسماء الألوان الشائعة
      IF v_word IN ('احمر', 'ازرق', 'اخضر', 'اسود', 'ابيض', 'اصفر', 'برتقالي', 'بنفسجي', 'وردي', 'رمادي', 'بني', 'سمائي') THEN
        RAISE NOTICE '⏭️ تجاهل لون: %', v_word;
        CONTINUE;
      END IF;
      
      -- تجاهل أسماء الأحجام
      IF v_word IN ('s', 'm', 'l', 'xl', 'xxl', 'xxxl', 'سمول', 'ميديم', 'لارج', 'اكس', 'دبل') THEN
        RAISE NOTICE '⏭️ تجاهل حجم: %', v_word;
        CONTINUE;
      END IF;
      
      -- إضافة الكلمة المتبقية
      RAISE NOTICE '✅ إضافة كلمة لأقرب نقطة دالة: %', v_word;
    ...
    RAISE NOTICE '📍 أقرب نقطة دالة النهائية: %', v_landmark;
  ELSE
    v_landmark := 'غير محدد';
    RAISE NOTICE '❌ لم يتم تحديد سطر عنوان، أقرب نقطة دالة = غير محدد';
  END IF;
