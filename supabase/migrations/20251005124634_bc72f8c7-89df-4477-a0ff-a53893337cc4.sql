-- إصلاح دالة process_telegram_order لاستخدام default_customer_name من إعدادات الموظف
...
DECLARE
  ...
  v_ai_order_id UUID;
BEGIN
  -- البحث عن الموظف المرتبط برقم الشات وقراءة default_customer_name من الإعدادات
  SELECT u.user_id, u.full_name, u.default_customer_name
  ...
  IF v_employee_id IS NULL THEN
    SELECT user_id, full_name, default_customer_name
    INTO v_employee_id, v_employee_name, v_default_customer_name
  ...
  -- تحديد الاسم الافتراضي للزبون: استخدم الإعدادات أو "زبون [اسم الموظف]"
  IF v_default_customer_name IS NOT NULL AND TRIM(v_default_customer_name) != '' THEN
    v_default_customer_name := TRIM(v_default_customer_name);
  ELSIF v_employee_name IS NOT NULL THEN
    v_default_customer_name := 'زبون ' || v_employee_name;
  ELSE
    v_default_customer_name := 'زبون تليغرام';
  END IF;
