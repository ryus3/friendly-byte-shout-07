...
-- تنظيف رسوم 5,000 لطلب ORD000008 (إن وجدت) وضبط إيراد 21,000
WITH main_source AS (
...
WHERE cm.cash_source_id = ms.id
  ...
  AND (cm.description ILIKE '%رسوم%' OR cm.description ILIKE '%delivery%' OR cm.reference_type ILIKE '%delivery%');
...
-- إزالة تكرارات مستحقات الموظف (-7,000) لطلب RYUS-299923 مع الإبقاء على الأحدث فقط
WITH v_main AS (
  SELECT id FROM public.cash_sources WHERE name = 'القاصة الرئيسية' ORDER BY created_at DESC LIMIT 1
), v_order AS (
  SELECT id FROM public.orders 
  WHERE tracking_number = 'RYUS-299923' 
     OR delivery_partner_order_id = 'RYUS-299923' 
     OR order_number = 'ORD000005'
  LIMIT 1
), dues AS (
  SELECT cm.id, cm.created_at,
         ROW_NUMBER() OVER (ORDER BY cm.created_at DESC, cm.id DESC) AS rn
  FROM public.cash_movements cm
  JOIN v_main m ON cm.cash_source_id = m.id
  JOIN v_order o ON cm.reference_id = o.id
...
-- ضمان ترتيب الإيراد قبل المستحقات بثانيتين إن لزم
DO $$
DECLARE
  v_main uuid;
  v_order uuid;
...
BEGIN
  ...
  ELSE
    ...
    -- حركة مستحقات الموظف (-7,000) بعد التنظيف
    ...
    IF v_rev_id IS NOT NULL AND v_dues_id IS NOT NULL THEN
      IF t_rev >= t_dues THEN
        ...
        RAISE NOTICE 'تم تقديم وقت الإيراد ليصبح قبل المستحقات: rev % قبل dues %', v_rev_id, v_dues_id;
      END IF;
    END IF;
  END IF;
END $$;

-- إعادة احتساب balance_before / balance_after لكل الحركات في القاصة الرئيسية من رأس مال 5,000,000
WITH main_source AS (
  SELECT id FROM public.cash_sources WHERE name = 'القاصة الرئيسية' ORDER BY created_at DESC LIMIT 1
), ordered AS (
