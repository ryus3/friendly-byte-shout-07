...
-- إعادة إنشاء الدالة الصحيحة مع منطق فهم المنتجات من كلمتين (مثل "سوت شيك")
...
BEGIN
  ...
  LOOP
    ...
    -- البحث عن المنتج (أولاً: منتجات من كلمتين، ثم كلمة واحدة)
    -- المحاولة الأولى: البحث عن منتجات من كلمتين متتاليتين (مثل "سوت شيك")
    FOR v_word_index IN 1..array_length(v_words, 1) - 1
    LOOP
      v_two_word_search := v_words[v_word_index] || ' ' || v_words[v_word_index + 1];
      
      SELECT p.id, p.name INTO v_found_product
      FROM products p 
      WHERE p.is_active = true 
        AND (
          lower(p.name) = v_two_word_search
          OR lower(p.name) LIKE v_two_word_search || '%'
          OR lower(p.name) LIKE '%' || v_two_word_search || '%'
        )
      ORDER BY 
        CASE WHEN lower(p.name) = v_two_word_search THEN 1
             WHEN lower(p.name) LIKE v_two_word_search || '%' THEN 2
             ELSE 3 END
      LIMIT 1;
      ...
      IF v_found_product.id IS NOT NULL THEN
        ...
        RAISE NOTICE '🎯 تم العثور على المنتج (كلمتين): %', v_found_product.name;
        EXIT;
      END IF;
    END LOOP;
    
    -- المحاولة الثانية: البحث عن منتج من كلمة واحدة (إذا لم نجد من كلمتين)
    IF v_found_product.id IS NULL THEN
      ...
      LOOP
        ...
        IF v_found_product.id IS NOT NULL THEN
          v_last_product_id := v_found_product.id;
          v_last_product_name := v_found_product.name;
          RAISE NOTICE '🎯 تم العثور على المنتج (كلمة واحدة): %', v_found_product.name;
          EXIT;
        END IF;
      END LOOP;
    END IF;
