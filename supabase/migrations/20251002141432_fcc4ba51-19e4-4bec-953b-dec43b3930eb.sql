-- إصلاح رسالة النجاح في process_telegram_order لعرض تفاصيل المنتجات
CREATE OR REPLACE FUNCTION public.process_telegram_order(p_message_text text, p_telegram_chat_id bigint, p_telegram_username text DEFAULT NULL::text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  v_success_message := '✅ تم استلام الطلب!' || E'\n\n' ||
    '🔹 ' || v_customer_name || E'\n' ||
    '📍 ' || v_customer_city || ' ' || v_customer_address || E'\n' ||
