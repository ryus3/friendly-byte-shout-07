-- إصلاح دالة process_telegram_order: إرجاع طريقة الحصول على اسم الموظف الصحيحة
...
BEGIN
  -- 1. البحث عن الموظف المرتبط بهذا chat_id
  ...
  -- استخدام telegram_code كاسم الموظف
  v_employee_name := COALESCE(v_employee_code, v_user_id::text);
  ...
  -- 3. استخراج اسم الزبون (أول كلمة أو كلمتين)
  ...
  -- إذا كان هناك منتجات غير متوفرة، إرجاع رسالة البدائل
