...
-- إنشاء دالة لتحديث profits table بـ settlement_invoice_id
CREATE OR REPLACE FUNCTION public.link_profits_to_settlement_invoices()
RETURNS void AS $$
DECLARE
...
BEGIN
  FOR settlement_record IN 
    SELECT id, employee_id, order_ids, settlement_date 
    FROM public.settlement_invoices
  LOOP
    -- ربط profits بفاتورة التسوية
    FOREACH order_id_item IN ARRAY settlement_record.order_ids
    LOOP
      UPDATE public.profits 
      SET status = 'settled',
          settled_at = settlement_record.settlement_date
      WHERE employee_id = settlement_record.employee_id
        AND order_id = order_id_item::uuid
        AND status != 'settled';
    END LOOP;
  END LOOP;
END;
...
-- تشغيل دالة الربط
SELECT public.link_profits_to_settlement_invoices();

RAISE NOTICE 'تم ترحيل جميع فواتير التسوية إلى جدول settlement_invoices بنجاح';