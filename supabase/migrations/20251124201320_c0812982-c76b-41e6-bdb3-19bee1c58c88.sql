-- ⚠️⚠️⚠️ CRITICAL RESTORATION ⚠️⚠️⚠️
-- استعادة extract_product_items_from_text الأصلية الكاملة (406 سطر)
-- النسخة الصحيحة من: 20251024202234_7921aefb-6b1e-43dd-9553-9c3761748cf0.sql
-- 
-- السبب: Migration 20251123235233 خرّب الدالة بتبسيطها إلى نسخة غير مكتملة
-- التاريخ: 2025-11-24
-- 
-- ⚠️ DO NOT SIMPLIFY THIS FUNCTION IN FUTURE MIGRATIONS
-- It contains critical AI order processing logic that MUST remain intact
...
BEGIN
  ...
  LOOP
    ...
    -- استبدال الكلمات المركبة للقياسات قبل تقسيم الكلمات
    ...
    -- تقسيم النص إلى كلمات بعد الاستبدال
    ...
    LOOP
      ...
      SELECT p.id::uuid, p.name, similarity(lower(p.name), v_search_text) as score
    ...
    IF v_found_product.id IS NULL THEN
      ...
      LOOP
        ...
        SELECT p.id::uuid, p.name INTO v_found_product
        FROM products_cache p 
    ...
    -- البحث عن اللون من products_cache
    FOREACH v_word IN ARRAY v_words
    LOOP
      SELECT (color_item->>'id')::uuid as id, color_item->>'name' as name
      INTO v_found_color
      FROM products_cache p, jsonb_array_elements(p.colors) AS color_item
      WHERE p.id = v_found_product.id
    ...
    -- البحث عن الحجم من products_cache
    FOREACH v_word IN ARRAY v_words
    LOOP
      SELECT (size_item->>'id')::uuid as id, size_item->>'name' as name
      INTO v_found_size
...
-- تعليق حماية لمنع التخريب مستقبلاً
COMMENT ON FUNCTION public.extract_product_items_from_text(text) IS 
'⚠️ CRITICAL: Full AI order extraction logic (406 lines) with Trigram/Bigram search, inventory checking, smart alternatives.
NEVER simplify or replace - stable version from 2025-10-24.
Last broken: 2025-11-23 by migration 20251123235233.
