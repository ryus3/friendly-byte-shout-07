-- تحديث دالة استخراج المنتجات - تحديث القياسات فقط
...
BEGIN
  ...
  LOOP
    ...
    -- البحث عن الحجم - المرحلة 1: الكلمات المركبة (كلمتان)
    FOR i IN 1..GREATEST(array_length(v_words, 1) - 1, 0)
    LOOP
      -- XL: اكس لارج
      IF v_words[i] = 'اكس' AND v_words[i+1] = 'لارج' THEN
        SELECT s.id, s.name INTO v_found_size
        FROM sizes s WHERE lower(s.name) = 'xl' LIMIT 1;
        IF v_found_size.id IS NOT NULL THEN
          v_size_requested := true;
          RAISE NOTICE '📏 تم العثور على الحجم (مركب): %', v_found_size.name;
          EXIT;
        END IF;
      
      -- XXL: اكسين لارج
      ELSIF v_words[i] = 'اكسين' AND v_words[i+1] = 'لارج' THEN
        SELECT s.id, s.name INTO v_found_size
        FROM sizes s WHERE lower(s.name) = 'xxl' LIMIT 1;
        IF v_found_size.id IS NOT NULL THEN
          v_size_requested := true;
          RAISE NOTICE '📏 تم العثور على الحجم (مركب): %', v_found_size.name;
          EXIT;
      ...
      -- XXXL: ثلاث اكسات
      ELSIF v_words[i] = 'ثلاث' AND v_words[i+1] = 'اكسات' THEN
        SELECT s.id, s.name INTO v_found_size
        FROM sizes s WHERE lower(s.name) = 'xxxl' LIMIT 1;
        IF v_found_size.id IS NOT NULL THEN
          v_size_requested := true;
          RAISE NOTICE '📏 تم العثور على الحجم (مركب): %', v_found_size.name;
          EXIT;
      ...
      -- XXXL: ثلاثة اكس
      ELSIF v_words[i] = 'ثلاثة' AND v_words[i+1] = 'اكس' THEN
        SELECT s.id, s.name INTO v_found_size
        FROM sizes s WHERE lower(s.name) = 'xxxl' LIMIT 1;
        IF v_found_size.id IS NOT NULL THEN
          v_size_requested := true;
      ...
      -- XXXL: ثلاثه اكس
      ELSIF v_words[i] = 'ثلاثه' AND v_words[i+1] = 'اكس' THEN
        SELECT s.id, s.name INTO v_found_size
        FROM sizes s WHERE lower(s.name) = 'xxxl' LIMIT 1;
        IF v_found_size.id IS NOT NULL THEN
          v_size_requested := true;
    ...
    -- البحث عن الحجم - المرحلة 2: الكلمات المفردة (فقط إذا لم نجد بالمرحلة 1)
    IF v_found_size.id IS NULL THEN
      ...
      LOOP
        ...
        WHERE lower(s.name) = v_word
           ...
           -- XXL
           OR (v_word = 'اكسين' AND lower(s.name) = 'xxl')
           OR (v_word = '2xxl' AND lower(s.name) = 'xxl')
           -- XXXL
           OR (v_word = '3xl' AND lower(s.name) = 'xxxl')
           -- Fallback
        ...
        IF v_found_size.id IS NOT NULL THEN
          ...
          EXIT;
        ELSE
          -- قائمة القياسات غير المتوفرة (للإشعار)
          IF v_word IN (
            'ميديم', 'لارج', 'سمول', 'اكس', 
            'اكسين', '2xxl',
            '3xl', 'ثلاث', 'ثلاثة', 'ثلاثه',
            'كبير', 'صغير', 'وسط',
            'xxxl', 'xxl', 'xl'
          ) THEN
            v_size_requested := true;
