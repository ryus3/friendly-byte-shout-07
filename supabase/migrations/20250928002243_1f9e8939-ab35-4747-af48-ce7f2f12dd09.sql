-- إصلاح منطق مطابقة الأحجام في دالة استخراج المنتجات
...
BEGIN
  ...
  LOOP
    ...
    -- البحث المباشر باستخدام شروط منفصلة
    IF v_normalized_word IN ('ميديم', 'وسط', 'medium') THEN
      -- البحث عن حجم M
      FOR v_size IN SELECT id, name FROM sizes WHERE lower(name) = 'm' LIMIT 1
      LOOP
        ...
        END IF;
      END LOOP;
    ELSIF v_normalized_word IN ('لارج', 'كبير', 'large') THEN
      -- البحث عن حجم L
      FOR v_size IN SELECT id, name FROM sizes WHERE lower(name) = 'l' LIMIT 1
      LOOP
        v_temp_size := jsonb_build_object('id', v_size.id, 'name', v_size.name);
        IF NOT (v_temp_size = ANY(SELECT jsonb_array_elements(v_found_sizes))) THEN
          v_found_sizes := v_found_sizes || jsonb_build_array(v_temp_size);
        ...
        END IF;
      END LOOP;
    ELSIF v_normalized_word IN ('سمول', 'صغير', 'small') THEN
      -- البحث عن حجم S
      FOR v_size IN SELECT id, name FROM sizes WHERE lower(name) = 's' LIMIT 1
      LOOP
        v_temp_size := jsonb_build_object('id', v_size.id, 'name', v_size.name);
        IF NOT (v_temp_size = ANY(SELECT jsonb_array_elements(v_found_sizes))) THEN
          v_found_sizes := v_found_sizes || jsonb_build_array(v_temp_size);
