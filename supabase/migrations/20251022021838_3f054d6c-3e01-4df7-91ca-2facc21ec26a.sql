-- إصلاح دالة extract_product_items_from_text - النسخة الذكية المُصلحة
-- هذا الإصلاح يحل مشكلة v_temp_product.priority عند NULL
...
BEGIN
  ...
  LOOP
    ...
    -- ========== البحث عن المنتج - النظام الذكي الجديد ==========
    
    -- STAGE 1: تطابق تام للنص الكامل
    v_temp_product := NULL;
    SELECT p.id, p.name INTO v_temp_product
    FROM products p 
    ...
    IF v_temp_product.id IS NOT NULL THEN
      ...
      RAISE NOTICE '🎯 [STAGE 1] تطابق تام للنص الكامل: %', v_found_product.name;
    END IF;
    
    -- STAGE 2: البحث عن كلمتين متتاليتين (أولوية أعلى من كلمة واحدة)
    IF v_found_product.id IS NULL AND array_length(v_words, 1) >= 2 THEN
      ...
      LOOP
        ...
        IF length(v_words[v_i]) < 2 OR length(v_words[v_i + 1]) < 2 
           OR v_words[v_i] ~ '^[0-9]+$' OR v_words[v_i + 1] ~ '^[0-9]+$' THEN
          CONTINUE;
        END IF;
        
        v_two_word_combo := v_words[v_i] || ' ' || v_words[v_i + 1];
        
        -- Priority 2: تطابق تام لكلمتين متتاليتين
        v_temp_product := NULL;
        SELECT p.id, p.name INTO v_temp_product
        FROM products p 
        WHERE p.is_active = true 
        ...
        IF v_temp_product.id IS NOT NULL THEN
          v_found_product := v_temp_product;
          RAISE NOTICE '🎯 [STAGE 2A] تطابق تام لكلمتين: %', v_found_product.name;
          EXIT; -- أفضل نتيجة ممكنة في هذه المرحلة
        END IF;
        
        -- Priority 3: اسم المنتج يحتوي على الكلمتين المتتاليتين
        v_temp_product := NULL;
        SELECT p.id, p.name INTO v_temp_product
        FROM products p 
        WHERE p.is_active = true 
        ...
        IF v_temp_product.id IS NOT NULL THEN
          v_found_product := v_temp_product;
          RAISE NOTICE '🎯 [STAGE 2B] يحتوي على كلمتين متتاليتين: %', v_found_product.name;
          -- لا نخرج هنا، قد نجد تطابقاً تاماً في التكرار التالي
        END IF;
      END LOOP;
    END IF;
    
    -- STAGE 3: البحث بكلمة واحدة (النظام الأصلي)
    IF v_found_product.id IS NULL THEN
      ...
      LOOP
        ...
        IF v_temp_product.id IS NOT NULL THEN
          v_found_product := v_temp_product;
          RAISE NOTICE '🎯 [STAGE 3] تطابق كلمة واحدة: %', v_found_product.name;
          EXIT;
        END IF;
      END LOOP;
    END IF;
    
    -- ========== نهاية البحث عن المنتج ==========
    
    -- تحديث آخر منتج تم العثور عليه
