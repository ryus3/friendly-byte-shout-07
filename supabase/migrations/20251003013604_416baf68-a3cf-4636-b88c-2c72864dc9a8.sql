-- إصلاح دالة get_unified_inventory_stats لاستخدام الأعمدة الصحيحة
...
BEGIN
  ...
  WHERE p.is_active = true
    ...
    -- فلترة الأقسام
    AND (v_has_full_department_access OR p.department_id IS NULL OR v_allowed_department_ids ? p.department_id::text)
    -- فلترة المواسم
    AND (v_has_full_season_access OR p.season_occasion_id IS NULL OR v_allowed_season_ids ? p.season_occasion_id::text)
    -- فلترة الألوان
