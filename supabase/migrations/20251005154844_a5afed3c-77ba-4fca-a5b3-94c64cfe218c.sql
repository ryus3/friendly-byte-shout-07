-- حذف جميع نسخ process_telegram_order الأخرى (11 نسخة)
DROP FUNCTION IF EXISTS public.process_telegram_order();
DROP FUNCTION IF EXISTS public.process_telegram_order(text);
...
DROP FUNCTION IF EXISTS public.process_telegram_order(text, text, bigint, integer, integer, text, text, text, boolean);
DROP FUNCTION IF EXISTS public.process_telegram_order(text, text, bigint, integer, integer, text, text, text, boolean, jsonb);
DROP FUNCTION IF EXISTS public.process_telegram_order(text, text, bigint, integer, integer, text, text, text, boolean, jsonb, uuid);

-- تعديل الدالة الصحيحة (7 معاملات) لإضافة أجور التوصيل للمبلغ الإجمالي
...
DECLARE
  ...
  v_item jsonb;
BEGIN
  -- الحصول على معلومات الموظف من employee_telegram_codes
  SELECT 
    etc.user_id,
  ...
  -- ✅ إضافة أجور التوصيل للمبلغ الإجمالي
