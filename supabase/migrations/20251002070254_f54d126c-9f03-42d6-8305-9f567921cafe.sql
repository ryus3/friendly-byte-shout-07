-- المرحلة 1: إضافة جدول أنماط التعلم الذكي
CREATE TABLE IF NOT EXISTS public.location_learning_patterns (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  pattern_text TEXT NOT NULL,
  normalized_pattern TEXT NOT NULL,
  resolved_city_id INTEGER REFERENCES public.cities_cache(id),
  resolved_region_id INTEGER REFERENCES public.regions_cache(id),
...
-- Index لتسريع البحث
CREATE INDEX IF NOT EXISTS idx_location_patterns_normalized ON public.location_learning_patterns(normalized_pattern);
CREATE INDEX IF NOT EXISTS idx_location_patterns_usage ON public.location_learning_patterns(usage_count DESC);
CREATE INDEX IF NOT EXISTS idx_location_patterns_confidence ON public.location_learning_patterns(confidence DESC);

-- RLS policies
ALTER TABLE public.location_learning_patterns ENABLE ROW LEVEL SECURITY;

CREATE POLICY "المديرون يديرون أنماط التعلم" ON public.location_learning_patterns
  FOR ALL USING (is_admin_or_deputy());

CREATE POLICY "المستخدمون يرون أنماط التعلم" ON public.location_learning_patterns
  FOR SELECT USING (auth.uid() IS NOT NULL);

-- المرحلة 2: إضافة delivery_fee لجدول ai_orders
ALTER TABLE public.ai_orders 
  ADD COLUMN IF NOT EXISTS delivery_fee NUMERIC DEFAULT 5000;

-- المرحلة 3: تحديث دالة process_telegram_order لإضافة رسوم التوصيل
...
BEGIN
  ...
  -- 2. قراءة الاسم الافتراضي من جدول profiles
  ...
  v_success_message := v_success_message || 
    '💵 المبلغ الإجمالي: ' || trim(to_char(v_total_amount, 'FM999,999')) || ' د.ع' || E'\n' ||
    '📦 (يشمل ' || trim(to_char(v_delivery_fee, 'FM999,999')) || ' د.ع رسوم توصيل)';

  RETURN jsonb_build_object(
    'success', true,
    'message', v_success_message,
    'customer_city', v_city_raw
  );
