...
-- تحديث trigger للطلبات ليتم استدعاء دالة تحديث الحجز تلقائياً
...
DECLARE
  result jsonb;
BEGIN
  -- عند تحديث حالة الطلب أو حالة التوصيل
  IF TG_OP = 'UPDATE' AND (
    OLD.status != NEW.status OR 
    OLD.delivery_status != NEW.delivery_status OR
    OLD.delivery_partner != NEW.delivery_partner
  ) THEN
    
    -- تحديث حالة حجز المخزون
    SELECT update_order_reservation_status(
      NEW.id,
      NEW.status,
      NEW.delivery_status,
    ...
    RAISE NOTICE 'تحديث حجز المخزون للطلب %: %', NEW.order_number, result;
  END IF;
