-- إصلاح دالة extract_product_items_from_text للبدائل الذكية
...
BEGIN
  ...
  LOOP
    ...
    ELSE
      ...
      IF v_word IN ('ميديم', 'لارج', 'سمول', 'اكس', 'دبل', 'كبير', 'صغير', 'وسط') THEN
        v_size_requested := true;
  ...
  -- رسالة الخطأ والبدائل
  IF v_color_requested AND v_found_color.id IS NULL THEN
    v_alternatives := format('❌ لم يتم إنشاء طلب!' || E'\n' ||
      'اللون "%s" غير متوفر لمنتج "%s"' || E'\n\n' ||
      '✅ 🎨 الألوان والأحجام المتوفرة:' || E'\n%s', 
      v_found_color.name, v_found_product.name, COALESCE(v_smart_alternatives, 'لا توجد بدائل متوفرة'));
  ELSIF v_size_requested AND v_found_size.id IS NULL THEN
    v_alternatives := format('❌ لم يتم إنشاء طلب!' || E'\n' ||
      'الحجم "%s" غير متوفر لمنتج "%s"' || E'\n\n' ||
      '✅ 🎨 الألوان والأحجام المتوفرة:' || E'\n%s', 
      v_found_size.name, v_found_product.name, COALESCE(v_smart_alternatives, 'لا توجد بدائل متوفرة'));
  ELSIF v_variant.id IS NULL OR v_variant.stock < v_quantity THEN
    v_alternatives := format('❌ لم يتم إنشاء طلب!' || E'\n' ||
  ...
  RAISE NOTICE '📝 رسالة البدائل الذكية: %', v_alternatives;
  
  -- إرجاع المنتج غير المتوفر مع البدائل الذكية
  RETURN jsonb_build_array(
    jsonb_build_object(
      'product_name', v_found_product.name,
