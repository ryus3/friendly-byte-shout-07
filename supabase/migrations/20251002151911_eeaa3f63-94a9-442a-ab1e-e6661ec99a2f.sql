-- إرجاع process_telegram_order للنسخة الصحيحة (145611) مع إضافة حفظ city_id و region_id
...
BEGIN
  ...
  IF array_length(v_city_region_array, 1) >= 2 THEN
    ...
    -- محاولة الحصول على city_id من cache
    SELECT id INTO v_city_id
    FROM cities_cache
    WHERE lower(name) = lower(v_city_name) AND is_active = true
    LIMIT 1;
    
    -- محاولة الحصول على region_id من cache
    IF v_city_id IS NOT NULL THEN
      SELECT id INTO v_region_id
      FROM regions_cache
      WHERE city_id = v_city_id 
        AND lower(name) = lower(v_region_name) 
        AND is_active = true
      LIMIT 1;
    END IF;
  ELSE
    -- محاولة الحصول على city_id حتى بدون منطقة
    SELECT id INTO v_city_id
    FROM cities_cache
    WHERE lower(name) = lower(v_city_raw) AND is_active = true
    LIMIT 1;
  END IF;
  ...
  -- 11. إدراج الطلب في ai_orders مع city_id و region_id
