...
BEGIN
  ...
  SELECT total_points, current_tier_id, customer_name 
  INTO customer_points, old_tier_id, customer_name
  ...
  IF new_tier_id != old_tier_id OR old_tier_id IS NULL THEN
    ...
    IF customer_name IS NOT NULL AND customer_name != '' THEN
      INSERT INTO public.notifications (
        ...
        'ترقية مستوى العميل',
        'تم ترقية العميل ' || customer_name || ' إلى مستوى ' || COALESCE(new_tier_name, 'غير محدد'),
        'loyalty_upgrade',
        jsonb_build_object(
...
-- إصلاح إحصائيات المدن - إنشاء دالة لتحديث إحصائيات المدن
