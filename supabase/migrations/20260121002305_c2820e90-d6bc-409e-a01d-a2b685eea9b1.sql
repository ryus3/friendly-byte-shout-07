-- Fix admin_manage_invoice_cron - use existing id or generate uuid
...
BEGIN
  ...
  -- Get existing ID or NULL
  SELECT id INTO v_existing_id FROM public.auto_sync_schedule_settings LIMIT 1;
  ...
  ELSE
    INSERT INTO public.auto_sync_schedule_settings (sync_times, enabled, timezone, updated_at)
    VALUES (ARRAY[p_morning_time, p_evening_time], p_enabled, 'Asia/Baghdad', NOW());
  END IF;
