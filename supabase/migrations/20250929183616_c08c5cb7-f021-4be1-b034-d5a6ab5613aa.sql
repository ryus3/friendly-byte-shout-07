-- إصلاح دالة استخراج المنتجات لترسل color و size بدلاً من color_name و size_name
...
BEGIN
  ...
  ELSIF v_variant.id IS NOT NULL AND v_variant.stock < v_quantity THEN
    -- الكمية المطلوبة غير متوفرة
    v_alternatives := format('❌ لم يتم إنشاء طلب!' || E'\n' ||
      'المنتج "%s" متوفر بكمية %s فقط (مطلوب %s)' || E'\n\n' ||
      '✅ الألوان والأحجام المتوفرة:' || E'\n%s', 
      v_found_product.name, v_variant.stock, v_quantity, COALESCE(v_smart_alternatives, 'لا توجد بدائل متوفرة'));
