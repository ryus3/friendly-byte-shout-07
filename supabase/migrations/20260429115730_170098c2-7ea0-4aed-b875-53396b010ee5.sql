-- 1) تحديث دالة upsert_alwaseet_invoice_list لاستخدام updated_at من API كتاريخ فاتورة، وحماية issued_at
...
BEGIN
  ...
  LOOP
    ...
    -- ✅ تاريخ الفاتورة الفعلي من شركة التوصيل (updated_at من API)
    v_issued_at := COALESCE(
      (v_invoice->>'updated_at')::timestamptz,
      (v_invoice->>'issued_at')::timestamptz,
      (v_invoice->>'issuedAt')::timestamptz,
      (v_invoice->>'created_at')::timestamptz,
      now()
    );
    
    v_received_at := CASE 
      WHEN v_received THEN COALESCE(
        (v_invoice->>'received_at')::timestamptz,
        v_issued_at
      )
      ELSE NULL 
    END;
    ...
    INSERT INTO delivery_invoices (
      external_id, partner, amount, orders_count, status, status_normalized,
      received, received_flag, issued_at, received_at, merchant_id,
      account_username, partner_name_ar, owner_user_id, raw,
      last_synced_at, last_api_updated_at
    ) VALUES (
      v_external_id,
      COALESCE(v_invoice->>'partner', 'alwaseet'),
      COALESCE((v_invoice->>'amount')::numeric, (v_invoice->>'merchant_price')::numeric, 0),
      COALESCE((v_invoice->>'orders_count')::integer, (v_invoice->>'ordersCount')::integer, (v_invoice->>'delivered_orders_count')::integer, 0),
      v_status, v_status_normalized, v_received, v_received,
      v_issued_at,
      v_received_at,
      v_invoice->>'merchant_id',
      v_invoice->>'account_username',
      v_invoice->>'partner_name_ar',
      v_owner_user_id, v_invoice, now(),
    ...
    ON CONFLICT (external_id, partner) DO UPDATE SET
      ...
      received_at = COALESCE(delivery_invoices.received_at, EXCLUDED.received_at),
      -- ✅ حماية issued_at من الكتابة فوقه (نأخذه من EXCLUDED فقط لو DB كان NULL أو نفس قيمة updated_at الجديدة الأحدث)
      issued_at = COALESCE(delivery_invoices.issued_at, EXCLUDED.issued_at),
  ...
  v_result := jsonb_build_object('success', true, 'upserted', v_upserted);
  RETURN v_result;
END;
$function$;

-- 2) ترحيل لمرة واحدة: تصحيح issued_at للفواتير التي ضُبط فيها وقت المزامنة بدل تاريخ الشركة
UPDATE delivery_invoices
SET issued_at = (raw->>'updated_at')::timestamptz
WHERE partner = 'alwaseet'
  AND raw ? 'updated_at'
  AND raw->>'updated_at' IS NOT NULL
  AND (raw->>'updated_at')::timestamptz < issued_at - interval '1 day';
