-- إضافة معامل اختياري للاسم الافتراضي في دالة process_telegram_order
CREATE OR REPLACE FUNCTION public.process_telegram_order(
  p_message_text text, 
  p_chat_id bigint,
  p_default_customer_name text DEFAULT 'عميل'
)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  LOOP
    ...
    IF v_found_region_id IS NULL THEN
      ...
      IF v_found_region_id IS NOT NULL THEN
        ...
        -- إذا وُجدت المنطقة أولاً، استخدم مدينتها
        IF v_found_city_id IS NULL AND v_region_city_id IS NOT NULL THEN
    ...
    -- جمع الكلمات المحتملة للأسماء (تجنب الألوان والأحجام والمدن والمناطق)
    IF v_word NOT IN (
  ...
  -- تحديد اسم العميل: استخدام الاسم الافتراضي إذا لم يتم العثور على اسم واضح
  IF array_length(v_names_words, 1) > 0 THEN
    -- التحقق من أن الكلمة ليست مدينة أو منطقة
    IF lower(v_names_words[1]) != lower(v_found_city_name) 
       AND lower(v_names_words[1]) != lower(v_found_region_name) THEN
      v_customer_name := initcap(v_names_words[1]);
    ELSE
      v_customer_name := p_default_customer_name;
    END IF;
  ELSE
    v_customer_name := p_default_customer_name;
  END IF;
  ...
  RAISE NOTICE '✅ انتهاء المعالجة بنجاح - العميل: %, المدينة: %, المنطقة: %', v_customer_name, v_found_city_name, v_found_region_name;
