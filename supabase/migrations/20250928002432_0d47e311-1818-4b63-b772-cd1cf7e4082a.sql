-- إضافة مرادفات أفضل للأحجام مع تحسين الطابقة
...
BEGIN
  ...
  -- البحث عن الأحجام مع مرادفات محسنة - إصلاح المنطق
  FOREACH v_word IN ARRAY v_words
  LOOP
    v_normalized_word := trim(lower(v_word));
    
    -- البحث في الأحجام مع معالجة خاصة للمرادفات
    IF v_normalized_word IN ('ميديم', 'وسط', 'm', 'medium') THEN
      FOR v_size IN SELECT id, name FROM sizes WHERE upper(name) = 'M' LIMIT 1
      LOOP
        ...
        IF NOT (v_temp_size = ANY(SELECT jsonb_array_elements(v_found_sizes))) THEN
          ...
          RAISE NOTICE 'تم العثور على الحجم: % (ID: %) للكلمة: %', v_size.name, v_size.id, v_normalized_word;
        END IF;
      END LOOP;
    ELSIF v_normalized_word IN ('لارج', 'كبير', 'l', 'large') THEN
      FOR v_size IN SELECT id, name FROM sizes WHERE upper(name) = 'L' LIMIT 1
      LOOP
        v_temp_size := jsonb_build_object('id', v_size.id, 'name', v_size.name);
        IF NOT (v_temp_size = ANY(SELECT jsonb_array_elements(v_found_sizes))) THEN
          v_found_sizes := v_found_sizes || jsonb_build_array(v_temp_size);
        ...
        END IF;
      END LOOP;
    ELSIF v_normalized_word IN ('سمول', 'صغير', 's', 'small') THEN
      FOR v_size IN SELECT id, name FROM sizes WHERE upper(name) = 'S' LIMIT 1
      LOOP
        v_temp_size := jsonb_build_object('id', v_size.id, 'name', v_size.name);
        IF NOT (v_temp_size = ANY(SELECT jsonb_array_elements(v_found_sizes))) THEN
          v_found_sizes := v_found_sizes || jsonb_build_array(v_temp_size);
