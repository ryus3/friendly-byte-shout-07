-- تعديل دالة معالجة طلبات تليغرام لإصلاح استخراج المدينة والمنطقة ورسالة النجاح
...
BEGIN
  ...
  -- 3. استخراج اسم الزبون من السطر الأول
  ...
  -- 6. استخراج المدينة والمنطقة من السطر الثاني
  v_city_raw := COALESCE(NULLIF(trim(split_part(p_message_text, E'\n', 2)), ''), 'غير محدد');
  
  -- تقسيم المدينة والمنطقة بشكل ذكي
  v_city_region_array := regexp_split_to_array(v_city_raw, '\s+');
  
  IF array_length(v_city_region_array, 1) >= 2 THEN
    v_city_name := v_city_region_array[1];  -- المدينة (بغداد)
    v_region_name := v_city_region_array[2];  -- المنطقة (الدولعي)
    v_city_raw := v_city_name || ' - ' || v_region_name;  -- بغداد - الدولعي
  END IF;

  -- 7. استخراج العنوان التفصيلي
  v_address := extract_actual_address(p_message_text);

  -- 8. إدراج الطلب في ai_orders مع created_by كـ UUID نصي
  ...
  -- 9. بناء رسالة النجاح بالصيغة المطلوبة
