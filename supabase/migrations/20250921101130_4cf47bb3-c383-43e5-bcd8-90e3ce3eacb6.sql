-- تحديث دالة إنشاء الطلبات لتجنب تكرار أرقام الطلبات
...
BEGIN
  ...
  -- إنشاء رقم طلب فريد مع تجنب التكرار
  LOOP
    v_order_number := 'TG-' || TO_CHAR(NOW(), 'YYYYMMDD') || '-' || 
      LPAD((EXTRACT(EPOCH FROM NOW()) + v_counter)::bigint::text, 8, '0');
    
    -- التحقق من عدم وجود رقم الطلب مسبقاً
    ...
    IF v_counter > 1000 THEN
      v_order_number := 'TG-' || gen_random_uuid()::text;
      EXIT;
    END IF;
  END LOOP;
