-- إرجاع دالة extract_product_items_from_text للنسخة السابقة مع إصلاح رسالة البدائل
  ...
  IF array_length(v_found_products, 1) > 0 THEN
    ...
    BEGIN
      ...
      ELSE
        ...
        -- جلب جميع المتغيرات المتوفرة لهذا المنتج بدون ORDER BY المشكل
        WITH available_variants AS (
          SELECT c.name as color_name, s.name as size_name
          FROM product_variants pv
