-- حذف الدالة القديمة وإعادة إنشائها مع الإصلاحات
DROP FUNCTION IF EXISTS public.extract_product_items_from_text(text);

-- إنشاء دالة محسّنة لاستخراج المنتجات مع عرض البدائل المتوفرة بشكل ذكي
...
BEGIN
  ...
  IF array_length(v_found_products, 1) > 0 THEN
    ...
    BEGIN
      ...
      ELSE
        -- المنتج غير متوفر - جلب البدائل المتوفرة
        RAISE NOTICE '❌ المنتج غير متوفر، سيتم جلب البدائل...';
        
        -- جلب جميع المتغيرات المتوفرة لهذا المنتج
        SELECT jsonb_object_agg(
          color_name, 
          sizes
        ) INTO v_available_colors_sizes
        FROM (
          SELECT c.name as color_name, jsonb_agg(DISTINCT s.name ORDER BY 
            CASE 
              WHEN s.name = 'XS' THEN 1
              WHEN s.name = 'S' THEN 2
              WHEN s.name = 'M' THEN 3
              WHEN s.name = 'L' THEN 4
              WHEN s.name = 'XL' THEN 5
              WHEN s.name = 'XXL' THEN 6
              ELSE 7
            END
          ) as sizes
          FROM product_variants pv
          ...
          LEFT JOIN inventory i ON pv.id = i.variant_id
          WHERE pv.product_id = v_product_id
            AND COALESCE(i.quantity, 0) > 0  -- فقط المتوفرة بالمخزون
